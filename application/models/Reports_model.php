...
class Reports_model extends CRM_Model
{
    ...
    function get_stats_chart_data($label, $where, $dataset_options, $year)
    {
        $chart = array(
            'labels' => array(),
            'datasets' => array(
    ...
    public function get_expenses_vs_income_report($year = '')
    {
        ...
        for ($m = 1; $m <= 12; $m++) {
            ...
            $this->db->select('amount,taxrate,tblexpenses.tax')->from('tblexpenses')->join('tbltaxes', 'tbltaxes.id = tblexpenses.tax', 'left')->where('MONTH(date)', $m)->where('YEAR(date)', $year);
    ...
    public function report_by_customer_groups()
    {
        ...
        if ($months_report != '') {
            ...
            if (is_numeric($months_report)) {
                ...
                $custom_date_select = '(tblinvoicepaymentrecords.date BETWEEN "' . $minus_months . '" AND "' . date('Y-m-d') . '")';
