...
{
    private $not_importable_leads_fields = array('id', 'source', 'assigned', 'status', 'dateadded', 'last_status_change', 'addedfrom', 'leadorder', 'date_converted', 'lost', 'junk', 'is_imported_from_email_integration', 'email_integration_uid', 'is_public', 'dateassigned', 'client_id', 'lastcontact', 'last_lead_status','from_form_id');
    ...
    public function save_form_data()
    {
        ...
        if ($this->db->affected_rows() > 0) {
            echo json_encode(array(
                'success' => true,
                'message' => _l('updated_successfuly', _l('web_to_lead_form'))
            ));
        } else {
            echo json_encode(array(
                'success' => false
    ...
    public function email_integration()
    {
        ...
        if ($this->input->post()) {
            $success = $this->leads_model->update_email_integration($this->input->post(NULL, FALSE));
            if ($success) {
                ...
                foreach ($ids as $id) {
                    if ($this->input->post('mass_delete')) {
                        ...
                        if($last_contact){
                            ...
                            $this->db->where('id',$id);
                            $this->db->update('tblleads',array('lastcontact'=>$last_contact));
                        }
                    }
                }
            }
